...
The SAM CLI installs dependencies defined in `tranmerewebcontactus/package.json`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
$ sam local invoke tranmerewebcontactus --event events/event.json
```
...
```bash
$ npm run build
$ sam deploy --stack-name ContactUs --capabilities CAPABILITY_IAM --s3-bucket tranmere-web-api
```
...
```bash
$ sam logs -n tranmerewebcontactusFunction --stack-name ContactUs --tail
```
...
```bash
$ aws cloudformation delete-stack --stack-name ContactUs
```
